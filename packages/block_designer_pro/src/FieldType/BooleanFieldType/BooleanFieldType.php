...
class BooleanFieldType extends FieldType
{
    ...
    public function getValidateFunctionContents()
    {
        if ($this->getRepeating()) {
            ...
            return 'if (in_array("' . $this->data['slug'] . '", $this->btFieldsRequired[\'' . $this->data['parent']['slug'] . '\']) && (!isset(' . $slug . ') || trim(' . $slug . ') == "" || !in_array(' . $slug . ', array(0, 1)))) {
                            $e->add(t("The %s field is required.", t("' . h($this->data['label']) . '"), t("' . h($this->data['parent']['label']) . '"), $' . $this->data['parent']['slug'] . '_k));
                        }';
        } else {
            return 'if (in_array("' . $this->data['slug'] . '", $this->btFieldsRequired) && (trim($args["' . $this->data['slug'] . '"]) == "" || !in_array($args["' . $this->data['slug'] . '"], array(0, 1)))) {
    ...
    {
        if (isset($this->data['default_value']) && trim($this->data['default_value']) != '' && in_array($this->data['default_value'], array(0, 1))) {
            if ($this->getRepeating()) {
                //return '$' . $this->data['parent']['slug'] . '_item_v["' . $this->data['slug'] . '"] = ' . $this->data['default_value'] . ';';
            } else {
    ...
    ' . parent::generateFormContent('select', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'options' => '(isset(' . $btFieldsRequired . ') && in_array(\'' . $this->data['slug'] . '\', ' . $btFieldsRequired . ') ? array() : array("" => "--" . t("Select") . "--")) + array(0 => ' . $no . ', 1 => ' . $yes . ')'), $repeating) . '
</div>';
    }
    ...
    {
        if ($this->getRepeating()) {
            return 'if (isset($' . $this->data['parent']['slug'] . '_item[\'' . $this->data['slug'] . '\']) && trim($' . $this->data['parent']['slug'] . '_item[\'' . $this->data['slug'] . '\']) != \'\' && in_array($' . $this->data['parent']['slug'] . '_item[\'' . $this->data['slug'] . '\'], array(0, 1))) {
            ...
            return 'if (!isset($args["' . $this->data['slug'] . '"]) || trim($args["' . $this->data['slug'] . '"]) == "" || !in_array($args["' . $this->data['slug'] . '"], array(0, 1))) {
            $args["' . $this->data['slug'] . '"] = \'\';
        }';
        }
    }
