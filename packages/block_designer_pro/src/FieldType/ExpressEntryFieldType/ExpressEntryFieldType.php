...
class ExpressEntryFieldType extends FieldType
{
    ...
    ' . parent::generateFormContent('express_entry_selector', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'attributes' => array('class' => 'express-entry-selector')), $repeating) . '
</div>';
        if ($repeating) {
    ...
    public function getAddEditFunctionContents()
    {
        ...
        if ($this->getRepeating()) {
            $lines[] = '$' . $this->data['parent']['slug'] . '[\'' . $this->data['slug'] . '_entity\'] = $entity_Manager->find(\'Concrete\Core\Entity\Express\Entity\', Database::connection()->fetchColumn(\'SELECT `id` FROM ExpressEntities WHERE `handle` = ?\', array(\'' . $this->data['entity_handle'] . '\')));';
        } else {
            $lines[] = '$this->set(\'' . $this->data['slug'] . '_entity\', $entity_Manager->find(\'Concrete\Core\Entity\Express\Entity\', Database::connection()->fetchColumn(\'SELECT `id` FROM ExpressEntities WHERE `handle` = ?\', array(\'' . $this->data['entity_handle'] . '\'))));';
        }
        return implode(PHP_EOL . '        ', $lines);
    }
