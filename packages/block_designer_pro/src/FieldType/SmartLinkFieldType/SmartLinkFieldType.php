...
class SmartLinkFieldType extends FieldType
{
	...
	protected $pkgVersionRequired = '2.5.1';
	protected $dbType = 'C';
	protected $uses = array('Core', 'File', 'Page', 'Permissions', 'URL');
	...
	public function getViewFunctionContents()
	{
		...
		if (!$repeating) {
			if (in_array('page', $options)) {
				$cases[] = 'case \'page\':
					if (!empty($this->' . $slugPage . ') && ($' . $slugPageC . ' = Page::getByID($this->' . $slugPage . ')) && !$' . $slugPageC . '->error && !$' . $slugPageC . '->isInTrash()) {
			...
			if (in_array('file', $options)) {
				$fileDownloadLink = isset($this->data['file_download']) && $this->data['file_download'] == '1' ? PHP_EOL . '							if (($c = Page::getCurrentPage()) && $c instanceof Page) {
		                        $' . $slugURL . ' = URL::to(\'/download_file\', $' . $slugObject . ', $c->getCollectionID());
		                    }' : null;
				$cases[] = 'case \'file\':
		...
		$this->set("' . $slugTitle . '", $' . $slugTitle . ');';
			...
			if (in_array('page', $options)) {
				$cases[] = 'case \'page\':
						if (!empty($' . $slugPage . ') && ($' . $slugPageC . ' = Page::getByID($' . $slugPage . ')) && !$' . $slugPageC . '->error && !$' . $slugPageC . '->isInTrash()) {
							$' . $slugObject . ' = $' . $slugPageC . ';
							$' . $slugURL . ' = $' . $slugPageC . '->getCollectionLink();
							if ($' . $slugTitle . ' == \'\') {
								$' . $slugTitle . ' = $' . $slugPageC . '->getCollectionName();
			...
			if (in_array('image', $options)) {
				$cases[] = 'case \'image\':
						if ($' . $slugImage . ' && ($' . $slugImageObject . ' = File::getByID($' . $slugImage . ')) && is_object($' . $slugImageObject . ')) {
							$' . $slugURL . ' = $' . $slugImageObject . '->getURL();
							$' . $slugObject . ' = $' . $slugImageObject . ';
							if ($' . $slugTitle . ' == \'\') {
								$' . $slugTitle . ' = $' . $slugImageObject . '->getTitle();
	...
	public function getViewContents()
	{
		...
		if (!empty($targetBlanks)) {
			$targetBlanksHtml = PHP_EOL . '	if (in_array($' . $slug . ', ' . var_export($targetBlanks, true) . ')) {
		$' . $slugAttributes . '[\'target\'] = \'_blank\';
	}';
		}
		return '<?php 
if (trim($' . $slugURL . ') != "") { ?>' . $this->data['prefix'] . '<?php 
	$' . $slugAttributes . ' = array();
	...
	public function getValidateFunctionContents()
	{
		...
		if ($repeating) {
			...
			return 'if ((in_array("' . $this->data['slug'] . '", ' . $btFieldsRequired . ') && (!isset(' . $slug . ') || trim(' . $slug . ') == "")) || (isset(' . $slug . ') && trim(' . $slug . ') != "" && !array_key_exists(' . $slug . ', $this->getSmartLinkTypeOptions(' . var_export($this->_options(), true) . ')))) {
							$e->add(t("The %s field has an invalid value.", ' . $label . '));
						} elseif (array_key_exists(' . $slug . ', $this->getSmartLinkTypeOptions(' . var_export($this->_options(), true) . '))) {
							switch (' . $slug . ') {
								' . implode(PHP_EOL . '				                ', $cases) . '	
							}
						}';
		...
		if ($repeating) {
			...
			foreach ($empties as $k => $v) {
				if (in_array(strtolower($k), $options)) {
					...
					' . $repeatingTabs . ${'slug' . ucfirst($k)} . ' = $' . $this->data['parent']['slug'] . '_item[\'' . $this->data['slug'] . '_' . ucfirst($k) . '\'];
	...
	{
		return '$(newField).find(".launch-tooltip").tooltip({container: "#ccm-tooltip-holder"});
        $.each($(newField).find(\'.ft-smart-link\'), function () {
            ...
            if ($(ftSmartLinkPage).length > 0) {
                $(ftSmartLinkPage).concretePageSelector({
                    ...
                    \'cID\': $(ftSmartLinkPage).attr(\'data-cID\')
                });
            }
        });
        setTimeout(function(){
	...
	{
		return '$this->set("' . $this->data['slug'] . '_Options", $this->getSmartLinkTypeOptions(' . var_export($this->_options(), true) . ', true));';
	}
	...
	$(\'#<?php  echo $' . $this->data['slug'] . '_ContainerID; ?> .ft-smart-link-type\').trigger(\'change\');
		...
		if (in_array('page', $options)) {
			...
			' . parent::generateFormContent('label', array('slug' => $this->data['slug'] . '_Page', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'label' => 'Page'), $repeating) . '
            ' . parent::generateFormContent('required', array(), $repeating) . '
            ' . parent::generateFormContent('page_selector', array('slug' => $this->data['slug'] . '_Page', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null), $repeating) . '
		</div>';
		}
		if (in_array('url', $options)) {
			$optionsHTML[] = '<div class="form-group hidden" data-link-type="url">
			' . parent::generateFormContent('label', array('slug' => $this->data['slug'] . '_URL', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'label' => 'URL'), $repeating) . '
            ' . parent::generateFormContent('required', array(), $repeating) . '
            ' . parent::generateFormContent('text', array('slug' => $this->data['slug'] . '_URL', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null), $repeating) . '
		</div>';
		}
		if (in_array('file', $options)) {
			$fileGroup = null;
			...
			$fileGroup .= PHP_EOL . '		    ' . parent::generateFormContent('label', array('slug' => $this->data['slug'] . '_File', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'label' => 'File'), $repeating) . '
            ' . parent::generateFormContent('required', array(), $repeating) . '
            ' . parent::generateFormContent('file', array('slug' => $slugFile, 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'postName' => $this->data['slug'] . '_File', 'bf' => $objectFile), $repeating) . '	
		</div>';
		...
		if (in_array('image', $options)) {
			...
			$imageGroup .= PHP_EOL . '			' . parent::generateFormContent('label', array('slug' => $this->data['slug'] . '_Image', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'label' => 'Image'), $repeating) . '
            ' . parent::generateFormContent('required', array(), $repeating) . '
            ' . parent::generateFormContent('image', array('slug' => $slugImage, 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'postName' => $this->data['slug'] . '_Image', 'bf' => $objectImage), $repeating) . '
		</div>';
...
<div class="ft-smart-link" id="<?php  echo $' . $this->data['slug'] . '_ContainerID; ?>">
	<div class="form-group">
	    ...
	    ' . parent::generateFormContent('select', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'options' => '$' . $this->data['slug'] . '_Options', 'attributes' => array('class' => 'form-control ft-smart-link-type')), $repeating) . '
	</div>
	
	<div class="form-group">
		<div class="ft-smart-link-options hidden" style="padding-left: 10px;">
			<div class="form-group">
				' . parent::generateFormContent('label', array('slug' => $this->data['slug'] . '_Title', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'label' => 'Title'), $repeating) . '
			    ' . parent::generateFormContent('text', array('slug' => $this->data['slug'] . '_Title', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null), $repeating) . '		
			</div>
...
' . $js;
	...
	{
		$options = array('page', 'file', 'image', 'url');
