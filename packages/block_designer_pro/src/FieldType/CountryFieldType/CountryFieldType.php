...
class CountryFieldType extends FieldType
{
    ...
    public function getAddEditFunctionContents()
    {
        if ($this->getRepeating()) {
            ...
            $' . $this->data['parent']['slug'] . '[\'' . $this->data['slug'] . '_countries\'] = array(\'\' => \'-- \' . t(\'None\') . \' --\') + $' . $this->data['parent']['slug'] . '[\'' . $this->data['slug'] . '_countries\'];
        }';
            return $return;
        } else {
            $return = $this->getCountriesLines();
            if (!$this->data['required']) {
                $return .= PHP_EOL . '        $' . $this->data['slug'] . '_countries = array(\'\' => \'-- \' . t(\'None\') . \' --\') + $' . $this->data['slug'] . '_countries;';
            }
    ...
    ' . parent::generateFormContent('select', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'options' => $counties), $repeating) . '
</div>';
    }
