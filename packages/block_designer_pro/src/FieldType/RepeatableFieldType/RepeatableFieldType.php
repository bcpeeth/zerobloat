...
class RepeatableFieldType extends FieldType
{
    ...
    {
        return array(
            $this->data['slug'] => $this->fieldsRequired,
        );
    }
    ...
    public function getSearchableContent()
    {
        ...
        $' . $this->data['slug'] . '_items = $db->fetchAll(\'SELECT * FROM ' . $this->getTableName() . ' WHERE bID = ? ORDER BY sortOrder\', array($this->bID));
    ...
    public function getTabs()
    {
        ...
        return array(
            array('form-' . $this->data['slug'] . '_items', h($this->data['label'])),
        );
    }
    ...
    public function getViewFunctionContents()
    {
        ...
        $lines[] = '$' . $this->data['slug'] . '_items = $db->fetchAll(\'SELECT * FROM ' . $this->getTableName() . ' WHERE bID = ? ORDER BY sortOrder\', array($this->bID));';
        $vLines = array();
        foreach ($this->fields as $ft) {
    ...
    public function getValidateFunctionContents()
    {
        ...
        $' . $this->data['slug'] . ' = array();' . (!empty($fieldValidationExtra) ? PHP_EOL . '        ' . implode(PHP_EOL . '                        ', $fieldValidationExtra) : null) . '
    ...
    var CCM_EDITOR_SECURITY_TOKEN = "<?php  echo Core::make(\'helper/validation/token\')->generate(\'editor\')?>";
        ...
        foreach ($this->fields as $ft) {
            if (method_exists($ft, 'getFormContents')) {
                $formContents .= '            ' . $ft->getFormContents();
            }
        }
        $sortableItemClasses = array('sortable-item');
        if (isset($this->data['no_collapse']) && is_string($this->data['no_collapse']) && $this->data['no_collapse'] == '1') {
            $sortableItemClasses[] = 'no-collapse';
        }
        $return .= '<?php  $repeatable_container_id = \'' . $this->data['btTable'] . '-' . $this->data['slug'] . '-container-\' . $identifier_getString; ?>
...
<script type="text/javascript">
    ...
    {
        $autoJsContents = array();
        $updateFunctions = null;
        foreach ($this->fields as $ft) {
            if (method_exists($ft, 'getRepeatableUpdateItemJS')) {
                $updateFunctions .= $ft->getRepeatableUpdateItemJS();
            }
            if (method_exists($ft, 'getAutoJsContents')) {
    ...
    public function getSaveFunctionContents()
    {
        ...
        return '$rows = $db->fetchAll(\'SELECT * FROM ' . $this->getTableName() . ' WHERE bID = ? ORDER BY sortOrder\', array($this->bID));
        $' . $this->data['slug'] . '_items = isset($args[\'' . $this->data['slug'] . '\']) && is_array($args[\'' . $this->data['slug'] . '\']) ? $args[\'' . $this->data['slug'] . '\'] : array();
        $queries = array();
                    ...
                    switch ($type) {
                        case \'update\':
                            foreach ($values as $id => $data) {
                                $db->update(\'' . $this->getTableName() . '\', $data, array(\'id\' => $id));
                            }
                            break;
                        case \'insert\':
                        ...
                        case \'delete\':
                            foreach ($values as $value) {
                                $db->delete(\'' . $this->getTableName() . '\', array(\'id\' => $value));
                            }
                            break;
                    }
                }
    ...
    public function getAddFunctionContents()
    {
        ...
        $lines[] = '        ' . '$this->set(\'' . $this->data['slug'] . '_items\', array());';
        foreach ($this->fields as $ft) {
    ...
    {
        $return = '$db->delete(\'' . $this->getTableName() . '\', array(\'bID\' => $this->bID));';
        foreach ($this->fields as $ft) {
    ...
    {
        $return = '$' . $this->data['slug'] . '_items = $db->fetchAll(\'SELECT * FROM ' . $this->getTableName() . ' WHERE bID = ? ORDER BY sortOrder\', array($this->bID));
        foreach ($' . $this->data['slug'] . '_items as $' . $this->data['slug'] . '_item) {
    ...
    public function getDbTables()
    {
        ...
        $tables = array(
            $this->getTableName($this->data) => array(
                'fields' => $fields
            )
        );
        foreach ($this->fields as $ft) {
