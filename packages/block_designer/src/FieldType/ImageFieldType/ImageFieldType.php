...
class ImageFieldType extends FieldType
{
        ...
        if (isset($this->data['output_src_only']) && $this->data['output_src_only'] == '1') {
            ...
            if (isset($this->data['responsive_image']) && $this->data['responsive_image'] == '1') {
                ...
                if (isset($this->data['thumbnail']) && $this->data['thumbnail'] == '1') {
                    ...
                    $lines[] = '$' . $slugTag . ' = Core::make(\'html/image\', array($' . $slug . '))->getTag();';
                }
    ...
    ' . parent::generateFormContent('image', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'attributes' => array('class' => 'ccm-file-selector ft-image-' . $this->data['slug'] . '-file-selector')), $repeating) . '
</div>';
        } else {
    ...
    ' . parent::generateFormContent('label', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'label' => $this->data['label'], 'description' => $this->data['description']), $repeating) . '
    ' . parent::generateFormContent('required', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'array' => $btFieldsRequired), $repeating) . '
    ' . parent::generateFormContent('image', array('slug' => 'ccm-b-' . $this->data['block_handle'] . '-' . $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'postName' => $this->data['slug'], 'bf' => '$' . $this->data['slug'] . '_o'), $repeating) . '
</div>';
        }
        if (isset($this->data['link']) && in_array($this->data['link'], array(1, 2))) {
            switch ($this->data['link']) {
