...
class StacksFieldType extends FieldType
{
    ...
    protected $uses = array('StackList', 'Stack', 'CollectionVersion', 'Database');
...
if (isset($' . $slug . ') && !empty($' . $slug . ')) { ?>' . $this->data['prefix'] . '<?php  foreach ($' . $slug . ' as $' . $slugStack . ') {
    ...
    public function getViewFunctionContents()
    {
        $repeating = $this->getRepeating();
        if ($repeating) {
            return '$' . $this->data['parent']['slug'] . '_item_v[\'' . $this->data['slug'] . '\'] = array();
        if ($' . $this->data['slug'] . '_entries = $db->fetchAll(\'SELECT * FROM ' . $this->getEntriesTableName($this->data) . ' WHERE bID = ? AND eID = ? ORDER BY sortOrder ASC\', array($this->bID, $' . $this->data['parent']['slug'] . '_item_v[\'id\']))) {
        ...
        if ($' . $this->data['slug'] . '_entries = $db->fetchAll(\'SELECT * FROM ' . $this->getEntriesTableName($this->data) . ' WHERE bID = ? ORDER BY sortOrder ASC\', array($this->bID))) {
            foreach ($' . $this->data['slug'] . '_entries as $' . $this->data['slug'] . '_entry) {
    ...
    public function getAddFunctionContents()
    {
        ...
        if ($repeating) {
            ...
            $lines[] = '$' . $this->data['slug'] . '_selected = array();
    ...
    {
        return '$' . $this->data['slug'] . '_entries = $db->fetchAll(\'SELECT * FROM ' . $this->getEntriesTableName($this->data) . ' WHERE bID = ? ORDER BY sortOrder ASC\', array($this->bID));
        foreach ($' . $this->data['slug'] . '_entries as $' . $this->data['slug'] . '_entry) {
    ...
    public function getSaveFunctionContents()
    {
        ...
        return '$' . $slugEntriesDB . ' = array();
        $' . $slugQueries . ' = array();
        if ($' . $slugEntries . ' = $db->fetchAll(\'SELECT * FROM ' . $entriesTableName . ' WHERE bID = ? ORDER BY sortOrder ASC\', array($this->bID))) {
                    ...
                    switch ($type) {
                        case \'update\':
                            foreach ($values as $id => $data) {
                                $db->update(\'' . $entriesTableName . '\', $data, array(\'id\' => $id));
                            }
                            break;
                        case \'insert\':
    ...
    public function getFormContents()
    {
        ...
        $attributes = $repeating ? array('class' => 'form-control select2_sortable') : array();
        $code = null;
    ...
    ' . parent::generateFormContent('select_multiple', array('slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'attributes' => $attributes, 'options' => $options, 'defaultValues' => 'isset($' . $this->data['slug'] . '_selected) ? $' . $this->data['slug'] . '_selected : array()'), $repeating) . '
</div>';
        if (!$repeating) {
