...
class BlockDesignerProcessor
{
    ...
    public static function updateFieldTypeCount($type, $repeatable = false)
    {
        if (!isset(self::$fieldTypeCounts[$type])) {
            self::$fieldTypeCounts[$type] = array(
                'normal'     => 0,
                'repeatable' => 0,
            );
        }
    ...
    public static function run($postData = array())
    {
        ...
        $autoJs = null;
        $extraFunction = array();
        $viewFields = array('<?php  defined("C5_EXECUTE") or die("Access Denied."); ?>');
        $formFields = array('<?php  defined("C5_EXECUTE") or die("Access Denied."); ?>');
        $tabs = array(array('form-basics', 'Basics'));
        $tabsFields = array('form-basics' => array());
        $dbTables = array();
        $fieldSlugs = array();
        $copyFiles = array();
        $assets = array();
        ...
        $controllerUsesArray = array('Concrete\Core\Block\BlockController', 'Core');
        $controllerVariables = array(
            ...
            'btIgnorePageThemeGridFrameworkContainer' => array(
                'type'  => 'protected',
        ...
        if (isset($postData['default_set']) && is_string($postData['default_set']) && trim($postData['default_set']) != '') {
            $controllerVariables['btDefaultSet'] = array(
                'type'  => 'protected',
                                    ...
                                    foreach ($ftAssets as $ftAssetFunctionName => $ftAssetArray) {
                                        if (isset($controllerFunctions[$ftAssetFunctionName])) {
                                            if (!isset($assets[$ftAssetFunctionName])) {
                                                $assets[$ftAssetFunctionName] = array();
                                            }
                                            foreach ($ftAssetArray as $ftAssetEntriesType => $ftAssetEntries) {
                                                // We only have these 2 functionalities with assets
                                                if (in_array($ftAssetEntriesType, array('require', 'register'))) {
                                                    if (!isset($assets[$ftAssetFunctionName][$ftAssetEntriesType])) {
                                                        $assets[$ftAssetFunctionName][$ftAssetEntriesType] = array();
                                                    }
                                                    foreach ($ftAssetEntries as $ftAssetEntry) {
                                                        $ftAddAsset = false;
                                                        if (isset($ftAssetEntry['type'])) {
                                                            if (!is_string($ftAssetEntry['type']) || trim($ftAssetEntry['type']) == '' || !in_array($ftAssetEntry['type'], array('css', 'javascript'))) {
        ...
        if (!empty(self::$errors)) {
            ...
            if (trim($autoJs) != '') {
                if (!isset($assets['composer'])) {
                    $assets['composer'] = array();
                }
                if (!isset($assets['composer']['register'])) {
                    $assets['composer']['register'] = array();
                }
                if (!isset($assets['composer']['require'])) {
                    $assets['composer']['require'] = array();
                }
                $assets['composer']['register'][] = array(
                    'type'     => 'javascript',
                    'handle'   => 'auto-js-' . $postData['block_handle'],
                    'filename' => 'blocks/' . $postData['block_handle'] . '/auto.js',
                );
                $assets['composer']['require'][] = array(
                    'type'   => 'javascript',
                    'handle' => 'auto-js-' . $postData['block_handle'],
            ...
            if (trim($autoCss) != '') {
                if (!isset($assets['addEdit'])) {
                    $assets['addEdit'] = array();
                }
                if (!isset($assets['addEdit']['register'])) {
                    $assets['addEdit']['register'] = array();
                }
                if (!isset($assets['addEdit']['require'])) {
                    $assets['addEdit']['require'] = array();
                }
                $assets['addEdit']['register'][] = array(
                    'type'     => 'css',
                    'handle'   => 'auto-css-' . $postData['block_handle'],
                    'filename' => 'blocks/' . $postData['block_handle'] . '/auto.css',
                );
                $assets['addEdit']['require'][] = array(
                    'type'   => 'css',
                    'handle' => 'auto-css-' . $postData['block_handle'],
            ...
            foreach ($assets as $assetFunctionName => $assetArray) {
                ...
                $assetArray = self::sortArrayByArray($assetArray, array('register', 'require'));
                    ...
                    foreach ($assetEntries as $assetEntry) {
                        ...
                        switch ($assetEntriesType) {
                            ...
                            case 'register':
                                ...
                                $args = isset($assetEntry['args']) ? $assetEntry['args'] : array();
                                $patterns = array("/\s+/", "/\s([?.!])/", '/,\)/', '/\',\'/', '/=>/', '/\),\'/');
                                $replacer = array('', '$1', ')', "', '", ' => ', '), \'');
                                $args = preg_replace($patterns, $replacer, var_export($args, true));
                                $assetCode = '$al->register(\'' . $assetEntry['type'] . '\', \'' . $assetEntry['handle'] . '\', \'' . $assetEntry['filename'] . '\', ' . $args . ', $this->pkg);';
                                break;
                        }
                        if (trim($assetCode) != '') {
                            $controllerFunctions[$assetFunctionName]['lines'][] = $assetCode;
                        }
                    }
                }
            }
            ...
            if (count($tabs) > 1) {
                ...
                foreach ($tabs as $tab) {
                    $tabsArray[] = self::getTabs(1) . 'array(\'' . $tab[0] . '-\' . $identifier_getString, t(\'' . $tab[1] . '\')' . (isset($tab[2]) && $tab[2] === true ? ', true' : null) . ')';
                }
                $formFields[] = '<?php  $tabs = array(' . PHP_EOL . implode(',' . PHP_EOL, $tabsArray) . PHP_EOL . ');
                    ...
                    $controllerFunctionsArray[] = $controllerFunction['type'] . ' function ' . $key . '(' . (isset($controllerFunction['variables']) && is_array($controllerFunction['variables']) && !empty($controllerFunction['variables']) ? implode(', ', $controllerFunction['variables']) : null) . ')
    {
        ' . implode(PHP_EOL . self::getTabs(2), $controllerFunction['lines']) . '
            ...
            foreach ($controllerVariables as $key => $classVariable) {
                $export = var_export($classVariable['value'], true);
                if (isset($classVariable['array_strip']) && $classVariable['array_strip'] === true) {
                    $patterns = array('(\d+\s=>)', "/\s+/", "/\s([?.!])/", '/,\)/', '/\',\'/', '/=>/', '/\),\'/');
                    $replacer = array('', '', '$1', ')', "', '", ' => ', '), \'');
