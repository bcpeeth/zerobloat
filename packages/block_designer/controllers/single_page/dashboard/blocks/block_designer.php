...
class BlockDesigner extends DashboardPageController
{
                    ...
                    try {
                        ...
                        if ($btID > 0 && $btsID) {
                            ...
                            if (array_key_exists($btsID, $blockTypeSets)) {
                                $db = Database::connection();
                                $order = 0;
                                if ($displayOrder = $db->fetchAssoc('SELECT (MAX(displayOrder) + 1) AS displayOrder FROM BlockTypeSetBlockTypes WHERE btsID = ?', array($btsID))) {
                                    $order = (int)$displayOrder['displayOrder'];
                                }
                                $insert_values = array($btID, $btsID, $order);
    ...
    {
        if (trim($blockHandle) == '') {
            $errors = array(t('There was no block handle found. Please enter a block handle.'));
        } else {
    ...
    public function on_start()
    {
        ...
        $al->register('css', 'responsive-tabs', 'css/responsiveTabs.css', array(), $this->packageHandle);
        $al->register('css', 'block-designer-view', 'css/block_designer.view.css', array(), $this->packageHandle);
        $al->register('javascript', 'responsive-tabs', 'js/jquery.responsiveTabs.js', array(), $this->packageHandle);
        $al->register('javascript', 'form-validator', 'js/form_validator/jquery.form-validator.min.js', array(), $this->packageHandle);
        $al->register('javascript', 'form-validator-file', 'js/form_validator/file.js', array(), $this->packageHandle);
        $al->register('javascript', 'form-validator-date', 'js/form_validator/date.js', array(), $this->packageHandle);
        $al->register('javascript', 'block-designer', 'js/jquery.blockDesigner.js', array(), $this->packageHandle);
        $al->register('javascript', 'block-designer-view', 'js/block_designer.view.js', array(), $this->packageHandle);
        $al->register('javascript', 'handlebars', 'js/handlebars-v4.0.5.js', array('minify' => false), $this->packageHandle);
        $al->register('javascript', 'handlebars-helpers', 'js/handlebars-helpers.js', array(), $this->packageHandle);
    }
    ...
    public function view($post_data = array())
    {
        ...
        foreach ($fieldTypes as $k => $fieldType) {
            if ($jsFile = $fieldType['class']->getFieldTypeJavascript()) {
                $al->register('javascript', 'field-type-' . $k . '-js', $jsFile, array(), $fieldType['class']->pkgHandle);
                $this->requireAsset('javascript', 'field-type-' . $k . '-js');
            }
            if ($cssFile = $fieldType['class']->getFieldTypeCss()) {
                $al->register('css', 'field-type-' . $k . '-css', $cssFile, array(), $fieldType['class']->pkgHandle);
    ...
    private function sortFieldTypes($fieldTypes = array())
    {
        $fileService = new File();
        $configFile = 'packages' . DIRECTORY_SEPARATOR . $this->packageHandle . DIRECTORY_SEPARATOR . 'config.json';
        $config = file_exists($configFile) ? (array)json_decode($fileService->getContents($configFile)) : array();
        $ftOrder = isset($config['ftOrder']) && is_array($config['ftOrder']) ? $config['ftOrder'] : array();
        $ftSort = isset($config['ftSort']) && is_string($config['ftSort']) ? $config['ftSort'] : 'normal';
        switch ($ftSort) {
            case 'asort':
            case 'arsort':
            case 'natsort':
                $fieldTypesNames = array();
